...
// oldBtcdHomeDir returns the OS specific home directory pointcoind used prior to
...
func oldBtcdHomeDir() string {
	...
	if appData != "" {
		return filepath.Join(appData, "pointcoind")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".pointcoind")
	}
...
// location prior to pointcoind version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to pointcoind
...
func upgradeDBPaths() error {
	...
	upgradeDBPathNet(filepath.Join(oldDbRoot, "pointcoind.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "pointcoind_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "pointcoind_regtest.db"), "regtest")
...
// upgradeDataPaths moves the application data from its location prior to pointcoind
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		pointcoindLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old pointcoind.conf into new location if needed.
		...
		if ohpEmpty {
			...
			pointcoindLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to pointcoind as new versions require it.
