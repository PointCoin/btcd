...
// list of commands that we recognise, but for which pointcoind has no support because
...
func genCertPair(certFile, keyFile string) error {
	...
	org := "pointcoind autogenerated cert"
...
func jsonAuthFail(w http.ResponseWriter, r *http.Request, s *rpcServer) {
	w.Header().Add("WWW-Authenticate", `Basic realm="pointcoind RPC"`)
	http.Error(w, "401 Unauthorized.", http.StatusUnauthorized)
}
...
var helpAddenda = map[string]string{
	"sendrawtransaction": `
NOTE: pointcoind does not currently support the "allowhighfees" parameter.`,
}

// getHelp text retreives help text from btcjson for the command in question.
// If there is any extra pointcoind specific information related to the given command
...
func handleStop(s *rpcServer, cmd btcjson.Cmd, closeChan <-chan struct{}) (interface{}, error) {
	s.server.Stop()
	return "pointcoind stopping.", nil
}
