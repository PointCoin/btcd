...
import (
	...
	"github.com/PointCoin/winsvc/eventlog"
	"github.com/PointCoin/winsvc/mgr"
	"github.com/PointCoin/winsvc/svc"
)

const (
	// svcName is the name of pointcoind service.
	svcName = "pointcoindsvc"
...
// logServiceStartOfDay logs information about pointcoind when the main server has
...
// pointcoindService houses the main service handler which handles all service
// updates and launching pointcoindMain.
type pointcoindService struct{}
...
// long-running pointcoindMain (which is the real meat of pointcoind), handles service
// change requests, and notifies the service control manager of changes.
func (s *pointcoindService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start pointcoindMain in a separate goroutine so the service can start
	...
	for {
		...
		case c := <-r:
			...
			case svc.Stop, svc.Shutdown:
				...
				// pointcoindMain to exit in the goroutine above which
...
// installService attempts to install the pointcoind service.  Typically this should
...
// removeService attempts to uninstall the pointcoind service.  Typically this should
...
// startService attempts to start the pointcoind service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &pointcoindService{})
	if err != nil {
